...
Customer_risk_streaming_df_enhanced = (
    customerRiskStreamingDF.join(emailAndBirthDayStreamingDF, expr("customer = email"))
)
(
    Customer_risk_streaming_df_enhanced
    .selectExpr("cast(customer as string) as key", "to_json(struct(*)) as value")
    .writeStream.format("kafka").option("kafka.bootstrap.servers", "kafka:19092")
    .option("topic", "stedi-enriched-data")
